...
2. **Users and Access → Integrations → In-App Purchase → generate a key.** Its Issuer ID, Key ID, and `.p8` become `APPLE_ISSUER_ID`, `APPLE_KEY_ID`, `APPLE_PRIVATE_KEY`. No environment switch is needed: the server looks a transaction up in production and falls back to sandbox, so a sandbox Apple Account works against a production deployment. `APPLE_ENVIRONMENT=sandbox` only reverses which is tried first, saving a round trip on a dev/staging box.
