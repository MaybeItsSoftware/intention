// Shared content + styles for App Store Connect marketing screenshots,
// reused across device-specific generators (iOS/iPad phone-frame mockups,
// macOS browser-window mockups). Keeping this in one place means every
// platform shows pixel-accurate copy and chip colors sourced from the real
// coaching/options UI (Intention Apple/Shared (Extension)/Resources/*.js).

import fs from 'node:fs';
import path from 'node:path';

// @font-face src pointed at file:// URLs silently fails to load under
// headless Chromium (document.fonts reports status "error"), so every
// screenshot was quietly falling back to Georgia/Times. Embedding the woff2
// as a base64 data URI loads reliably instead.
...
// chip UI, so the mockup chips are pixel-accurate to the real app rather
// than invented.
...
export const icon = (key, size = '1.6cqw') => {
...
export const chip = (key, selected) =>
...
// scale with whatever frame holds them, phone or browser window) ----------

export const COACHING_CSS = `
...
export const coachingScreen = ({ eyebrowSub, stats, messages, closeLabel = 'Close tab' }) => `
  <div class="coach-col">
...
export const screen1 = coachingScreen({
...
export const screen3 = coachingScreen({
...
export const rowItem = (label, minutes = 10) => `
