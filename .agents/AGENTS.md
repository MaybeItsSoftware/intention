...
- `hosted` — an active subscription; calls go to `server/` under Intention's key. **This is the default and the only thing onboarding offers on any build a store reviews.**
