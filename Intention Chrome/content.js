...
chrome.storage.local.get(['blockedDomains', 'setupComplete'], (result) => {
  ...
  console.log(INT_LOG, 'storage read', { host, domains, setupComplete });
  ...
  // Fail safe: if the background never responds (e.g. Safari's background page
  // hadn't woken yet), still show the gate instead of leaving the page hidden
  // and stuck loading.
  const fallbackTimer = setTimeout(() => showGate('timeout — background did not respond'), 1200);

  try {
    chrome.runtime.sendMessage({ action: 'getSession' }, (response) => {
      clearTimeout(fallbackTimer);
      if (chrome.runtime.lastError) {
        showGate('lastError: ' + chrome.runtime.lastError.message);
        return;
      }
      console.log(INT_LOG, 'getSession response', response);
      if (response && response.session) {
        ...
        showGate('no active session');
      }
    });
  } catch (e) {
    clearTimeout(fallbackTimer);
    showGate('sendMessage threw: ' + (e && e.message));
  }
});
