...
## Getting a balance back

A top-up is a consumable. Its receipt is consumed at purchase, Play's INAPP
query stops returning it, and Apple's `currentEntitlements` excludes
...
- **The account id survived.** Apple keeps it in the Keychain
  ...
  Backup restores. `POST /v1/entitlement/recover` takes that id and mints a
  token if — and only if — a `balance:` record already exists for it. It is
  *presence*, not a positive balance: someone who spent down to exactly zero
  still recovers, so their next top-up lands in the same balance instead of a
  new one. The account id is the whole credential here, because no other proof
  survives; 30 requests/hour/IP plus a 10-miss/hour/IP lockout is what stands
...
The bearer is only half of it, because the path the override exists for is
also the path with no bearer on it. Tokens age out at
`INTENTION_TOKEN_MAX_LIFETIME_MS`; `billing.js`'s `refreshEntitlement` then
falls back to re-verifying the stored receipt, and that call sends no
`Authorization` header. So **for a transaction that was already credited, the
response names the subject on the credit record**, not the attested one. The
credit record is the authority on where money landed — the refund clawback
already reads it the same way — and it needs no live credential to consult.
That is not a new way to read a stranger's balance: an ordinary receipt
carries the buyer's own `appAccountToken`, so possessing one already resolves
to their subject.

**Residual gap, deliberately not closed.** A *new* top-up from a recovered
device still needs the client to attach its bearer, because a never-before-
